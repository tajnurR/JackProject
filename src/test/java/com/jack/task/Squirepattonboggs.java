...
public class Squirepattonboggs {
...
//		driver.manage().deleteAllCookies();
		...
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='contact-details-title']/h1")));

	//	Name
		try {
			String name = driver.findElement(By.xpath("//div[@class='contact-details-title']/h1")).getText();
...
//		//	Title
		try {
			String title = driver.findElement(By.xpath("//div[@class='contact-details-title']/h2")).getText();
		...
		//	Email
		try {
			String email = driver.findElement(By.xpath("//div[@class='bio-links']/a[@class='contact-email']")).getText();
...
//		Location
		try {
			String main = "";
			int locationSize = driver.findElements(By.xpath("//div[@class='contact-offices']/ul/li[1]/div[contains(@class,'office-name')]")).size();
			for (int i = 1; i < locationSize+1; i++) {
				String location = driver.findElement(By.xpath("(//div[@class='contact-offices']/ul/li[1]/div[contains(@class,'office-name')])["+i+"]")).getText();
				if (i<=1) {
...
//		Bio
		try {
			...
			StringBuilder bio = new StringBuilder();
				bio.append(driver.findElement(By.xpath("//div[@class='content centered']")).getAttribute("innerHTML"));
...
//		practice
		...
		try {
			driver.findElement(By.xpath("//li[@class='ng-scope is-selected']//a[@class='tile ng-binding'][normalize-space()='Credentials']")).click();
			Thread.sleep(2000);
		} catch (Exception e) {
			// TODO: handle exception
		}
		...
		try {
			String main = "";
			int practiceSize = driver.findElements(By.xpath("//div[contains(@class,'related-services')]/ul/li")).size();
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("(//div[contains(@class,'related-services')]/ul/li)["+i+"]")).getText();
				if (i<=2) {
...
//		Admissions
		try {
			String main = "";
			int LangSize = driver.findElements(By.xpath("//h3[normalize-space()='Admissions']/following::ul[1]/li")).size();
			
			for (int i = 1; i < LangSize+1; i++) {
				String bar = driver.findElement(By.xpath("//h3[normalize-space()='Admissions']/following::ul[1]/li["+i+"]")).getAttribute("innerHTML");
...
//		Education
		try {
			...
			for (int i = 1; i < eduSize+1; i++) {
				String edu = driver.findElement(By.xpath("//h3[normalize-space()='Education']/following-sibling::ul[1]/li["+i+"]")).getAttribute("innerHTML");
				if (i<=5) {
...
//		Language
		try {
			...
			String main = "";
			int eduSize = driver.findElements(By.xpath("//h3[normalize-space()='Languages']/following-sibling::ul[1]/li")).size();
			for (int i = 1; i < eduSize+1; i++) {
				String edu = driver.findElement(By.xpath("//h3[normalize-space()='Languages']/following-sibling::ul[1]/li["+i+"]")).getAttribute("innerHTML");
					main+=";"+edu;
			}
...
//		Image 
		try {
			String img = driver.findElement(By.xpath("//div[contains(@class,'contact-details-image')]/img")).getAttribute("src");
