...
//		utitle.setCellData(fileOut, sheetOut, row, 0, link);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='profile_title']/h2[@id='full_name']")));
	...
	//	Name
		try {
			String name = driver.findElement(By.xpath("//div[@class='profile_title']/h2[@id='full_name']")).getText();
...
//		//	Title
		try {
			String title = driver.findElement(By.xpath("//div[@class='profile_title']/h2[@id='designation']")).getText();
...
//		//	Email
		try {
			String email = driver.findElement(By.xpath("//a[@class='email-custom-btn']")).getText();
...
//		
		//	Phone
		try {
			String phone = driver.findElement(By.xpath("//div[@class='phone_numbers']/p[1]")).getText();
			sm.setPhone(phone);
		} catch (Exception e) {
...
//		Location
		try {
			String main = "";
			int locationSize = driver.findElements(By.xpath("//div[@class='address']/a")).size();
			for (int i = 1; i < locationSize+1; i++) {
				String location = driver.findElement(By.xpath("(//div[@class='address']/a)["+i+"]")).getText();
				if (i<=1) {			
...
//		practice
		try {
			...
			String main = "";
			int practiceSize = driver.findElements(By.xpath("//h3[normalize-space()='PRACTICE']/following-sibling::a")).size();
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("(//h3[normalize-space()='PRACTICE']/following-sibling::a)["+i+"]")).getText();
				if (i<=2) {
//					
					if (i==1) {
...
//		Admissions
		try {
			...
			String main = "";
			int LangSize = driver.findElements(By.xpath("//h3[normalize-space()='ADMISSIONS']/following-sibling::p")).size();
			
			for (int i = 1; i < LangSize+1; i++) {
				String bar = driver.findElement(By.xpath("(//h3[normalize-space()='ADMISSIONS']/following-sibling::p)["+i+"]")).getText();
...
//		Education
		try {
			...
			String main = "";
			int eduSize = driver.findElements(By.xpath("(//h3[normalize-space()='EDUCATION']/following-sibling::p)")).size();
			for (int i = 1; i < eduSize+1; i++) {
				String edu = driver.findElement(By.xpath("(//h3[normalize-space()='EDUCATION']/following-sibling::p)["+i+"]")).getText();
				if (i<=5) {
					if (i==1) {
						sm.setEdu1(edu);
					}else if (i==2) {
...
//		Bio
		try {
			StringBuilder bio = new StringBuilder();
				bio.append(driver.findElement(By.xpath("(//section[@id='biography-section']//div[@class='row'])[2]")).getText());
//				try {
//
