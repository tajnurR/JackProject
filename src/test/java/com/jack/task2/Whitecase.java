...
//	String sheetName = "Sheet1";
	String filename = "D:\\Fiverr Work\\Jack Project\\whitecase\\input.xlsx";
	String sheetName = "Sheet1";
...
//		driver.manage().deleteAllCookies();
		...
		sm.setLink(link);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h1[@class='page-header']/span")));
	...
	//	Name
		try {
			String name = driver.findElement(By.xpath("//h1[@class='page-header']/span")).getText();
			sm.setName(name);
		} catch (Exception e) {
		...
		//	Title
		try {
			String title = driver.findElement(By.xpath("//div[@class='heading__bio-info']")).getText();
			sm.setTitle(title);
		} catch (Exception e) {
...
////		//	Email
		try {
			String email = driver.findElement(By.xpath("(//div[contains(@class,'field--name-field-email')]/a[contains(@href,'mailto:')])[1]")).getAttribute("href");
			sm.setEmail(email);
		} catch (Exception e) {
...
//		//	Phone
		try {
			String phone = driver.findElement(By.xpath("(//ul[contains(@class,'field--name-field-phone')]/li/a[contains(@href,'tel:')])[1]")).getText();
			sm.setPhone(phone);
		} catch (Exception e) {
...
//		}
		...
		try {
			StringBuilder bio = new StringBuilder();
					bio.append(driver.findElement(By.xpath("(//div[contains(@class,'field--name-body')])")).getText());
...
//		Admissions
		try {
			String main = "";
			int LangSize = driver.findElements(By.xpath("(//div[contains(@class,'field--name-field-admissions')]/div[@class='field--items']/div)")).size();
				for (int i = 1; i < LangSize+1; i++) {
					String bar = driver.findElement(By.xpath("(//div[contains(@class,'field--name-field-admissions')]/div[@class='field--items']/div)["+i+"]")).getText();
					if (i<=1) {				
						sm.setBar1(bar);
					}else {
						main+=";"+bar;
...
//		Education
		try {
			...
			String main = "";
			int eduSize = driver.findElements(By.xpath("(//div[contains(@class,'field--name-field-education')]/div[@class='field--items']/div)")).size();
			for (int i = 1; i < eduSize+1; i++) {
				String edu = driver.findElement(By.xpath("(//div[contains(@class,'field--name-field-education')]/div[@class='field--items']/div)["+i+"]")).getText();
				if (i<=5) {
					if (i==1) {
						sm.setEdu1(edu);
					}else if (i==2) {
...
//		Language
		try {
			String main = "";
			int eduSize = driver.findElements(By.xpath("(//div[contains(@class,'field--name-field-spoken-languages')]/div[@class='field--items']/div)")).size();
			for (int i = 1; i < eduSize+1; i++) {
				String edu = driver.findElement(By.xpath("(//div[contains(@class,'field--name-field-spoken-languages')]/div[@class='field--items']/div)["+i+"]")).getText();
					main+=";"+edu;
			}
			sm.setLanguage(main);
...
////		Image 
		try {
			String img = driver.findElement(By.xpath("//div[@class='focuspoint']/picture/img")).getAttribute("src");
			sm.setImg(img);
		} catch (Exception e) {
			// TODO: handle exception
		}
...
//		All
		try {
			...
			exp.append(driver.findElement(By.xpath("//div[contains(@class,'field--name-field-personal-projects')]")).getText());
			sm.setExp(exp);
		} catch (Exception e) {
			// TODO: handle exception
		}
...
//		practice
		try {
			...
			String main = "";
			int practiceSize = driver.findElements(By.xpath("(//div[contains(@class,'field--name-field-related-services')]/div/div)")).size();
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("(//div[contains(@class,'field--name-field-related-services')]/div/div)["+i+"]")).getText();
				if (i<=2) {
					if (i==1) {
	...
	public void saveList() throws Exception {
		...
		mapper.writeValue(new File("D:\\Fiverr Work\\Jack Project\\whitecase\\row.json"), lists);
