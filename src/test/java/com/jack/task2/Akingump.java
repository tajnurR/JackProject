...
//	String sheetName = "Sheet1";
	String filename = "D:\\Fiverr Work\\Jack Project\\Akingump\\input.xlsx";
...
//		driver.manage().deleteAllCookies();
		...
		sm.setLink(link);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h2[@class='tablet-show']")));
	...
	//	Name
		try {
			String name = driver.findElement(By.xpath("//h2[@class='tablet-show']")).getText();
			sm.setName(name);
		} catch (Exception e) {
...
//		}
		...
		try {
			String main = "";
			int locationSize = driver.findElements(By.xpath("(//div[@class='attorney-detail']//a[contains(@href,'/locations/')])")).size();
			for (int i = 1; i < locationSize+1; i++) {
				String location = driver.findElement(By.xpath("(//div[@class='attorney-detail']//a[contains(@href,'/locations/')])["+i+"]")).getText();
				if (i<=1) {			
...
//		practice
		try {
			...
			String main = "";
			int practiceSize = driver.findElements(By.xpath("(//h2[normalize-space()='Areas of Experience']/following-sibling::div[@class='list-wrapper']/ul/li)")).size();
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("(//h2[normalize-space()='Areas of Experience']/following-sibling::div[@class='list-wrapper']/ul/li)["+i+"]")).getText();
				if (i<=2) {
					if (i==1) {
...
//			}
			...
			exp.append(driver.findElement(By.xpath("(//h3[normalize-space()='Key Experience']/following-sibling::div)[1]")).getText());
			sm.setExp(exp);
		} catch (Exception e) {
			// TODO: handle exception
		}
		...
		try {
			StringBuilder bio = new StringBuilder();
				try {
					bio.append(driver.findElement(By.xpath("//div[@data-id='js-bio-main-content'][1]")).getAttribute("innerHTML"));
					bio.append(driver.findElement(By.xpath("//div[@data-id='js-bio-main-content'][2]")).getAttribute("innerHTML"));
				} catch (Exception e) {
					// TODO: handle exception
				}
			sm.setBio(bio);
...
//		Admissions
		try {
			String main = "";
			int LangSize = driver.findElements(By.xpath("(//h2[normalize-space()='Bar Admissions']/parent::aside/div[@class='list-wrapper']/ul/li)")).size();
				for (int i = 1; i < LangSize+1; i++) {
					String bar = driver.findElement(By.xpath("(//h2[normalize-space()='Bar Admissions']/parent::aside/div[@class='list-wrapper']/ul/li)["+i+"]")).getText();
					if (i<=1) {				
						sm.setBar1(bar);
					}else {
						main+=";"+bar;
...
//				
//			String main = "";
//			int eduSize = driver.findElements(By.xpath("(//h3[normalize-space()='Degrees']/parent::li/following-sibling::li[@class='header']/preceding-sibling::li)")).size();
//			for (int i = 1; i < eduSize+1; i++) {
//				String edu = driver.findElement(By.xpath("(//h3[normalize-space()='Degrees']/parent::li/following-sibling::li[@class='header']/preceding-sibling::li)["+i+"]")).getText();
//				if (i<=5) {
//					if (i==1) {
//						sm.setEdu1(edu);
//					}else if (i==2) {
...
//		try {
//			String main = "";
//			int eduSize = driver.findElements(By.xpath("(//h3[normalize-space()='Languages']/following-sibling::p)")).size();
//			for (int i = 1; i < eduSize+1; i++) {
...
//				
//			String main = "";
//			int eduSize = driver.findElements(By.xpath("(//h3[normalize-space()='Clerkships']/parent::li/following-sibling::li)")).size();
//			for (int i = 1; i < eduSize+1; i++) {
//				String edu = driver.findElement(By.xpath("(//h3[normalize-space()='Clerkships']/parent::li/following-sibling::li)["+i+"]")).getText();
//					main+=";"+edu;
//			}
...
////		Image 
		try {
			String img = driver.findElement(By.xpath("//div[@class='visual']/img")).getAttribute("src");
			sm.setImg(img);
		} catch (Exception e) {
			// TODO: handle exception
		}
	...
	public void saveList() throws Exception {
		...
		mapper.writeValue(new File("D:\\Fiverr Work\\Jack Project\\Akingump\\row.json"), lists);
	}
