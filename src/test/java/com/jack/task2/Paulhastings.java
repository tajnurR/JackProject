...
//	String sheetName = "Sheet1";
	String filename = "D:\\Fiverr Work\\Jack Project\\Paulhastings\\input.xlsx";
...
//		driver.manage().deleteAllCookies();
		...
		sm.setLink(link);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h1[contains(@class,'qtph-profprofile-name-txt')]")));
	...
	//	Name
		try {
			String name = driver.findElement(By.xpath("//h1[contains(@class,'qtph-profprofile-name-txt')]")).getText();
			sm.setName(name);
		} catch (Exception e) {
...
//		//	Email
		try {
			String email = driver.findElement(By.xpath("//div[contains(@class,'qtph-profprofile-email-txt')]")).getText();
			sm.setEmail(email);
		} catch (Exception e) {
...
//		
		//	Phone
		try {
			String phone = driver.findElement(By.xpath("//div[contains(@class,'qtph-profprofile-primaryoffice-txt')]")).getAttribute("innerHTML");
			sm.setPhone(phone);
		} catch (Exception e) {
...
//		practice
		try {
			...
			String main = "";
			int practiceSize = driver.findElements(By.xpath("(//div[contains(@class,'qtph-profprofile-sidebar')]/a/div)")).size();
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("(//div[contains(@class,'qtph-profprofile-sidebar')]/a/div)["+i+"]")).getText();
				if (i<=2) {
//					
					if (i==1) {
...
//		All
		try {
			...
			exp.append(driver.findElement(By.xpath("//div[contains(@class,'qtph-profprofile-sidebar')]")).getAttribute("innerHTML"));
			sm.setExp(exp);
		} catch (Exception e) {
			// TODO: handle exception
		}
		...
		try {
			StringBuilder bio = new StringBuilder();
				bio.append(driver.findElement(By.xpath("//h1[normalize-space()='Overview']/following-sibling::div[1]")).getText());
			sm.setBio(bio);
...
////		Image 
		try {
			String img = driver.findElement(By.xpath("//img[contains(@src,'https://images')]")).getAttribute("src");
			sm.setImg(img);
		} catch (Exception e) {
			// TODO: handle exception
		}
	...
	public void saveList() throws Exception {
		...
		mapper.writeValue(new File("D:\\Fiverr Work\\Jack Project\\Paulhastings\\row.json"), lists);
	}
