...
//		driver.manage().deleteAllCookies();
		...
		sm.setLink(link);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='layout-a row']/div/h1")));
	...
	//	Name
		try {
			String name = driver.findElement(By.xpath("//div[@class='layout-a row']/div/h1")).getText();
			sm.setName(name);
...
//		//	Title
		try {
			String title = driver.findElement(By.xpath("//header[@class='bio-bar-header']/span[@class='h3']")).getText();
			sm.setTitle(title);
		} catch (Exception e) {
...
//		//	Email
		try {
			String email = driver.findElement(By.xpath("//section[@class='bio-bar']/ul/li/a[contains(@href,'mailto:')]")).getText();
			sm.setEmail(email);
		} catch (Exception e) {
...
//		
		//	Phone
		try {
			String phone = driver.findElement(By.xpath("//section[@class='bio-bar']/ul/li/a[contains(@href,'tel:')]")).getText();
			sm.setPhone(phone);
		} catch (Exception e) {
...
//		Location
		try {
			String main = "";
			int locationSize = driver.findElements(By.xpath("(//ul[contains(@class,'multiplePhones')]/li/a[contains(@href,'tel:')]/span)|//header[@class='bio-bar-header']/span[@class='h3']/span")).size();
			for (int i = 1; i < locationSize+1; i++) {
				String location = driver.findElement(By.xpath("(//ul[contains(@class,'multiplePhones')]/li/a[contains(@href,'tel:')]/span)["+i+"]|//header[@class='bio-bar-header']/span[@class='h3']/span["+i+"]")).getText();
				if (i<=1) {			
...
//		practice
		try {
			...
			String main = "";
			int practiceSize = driver.findElements(By.xpath("//h1[normalize-space()='Practice Areas']/parent::header/parent::section/ul/li")).size();
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("(//h1[normalize-space()='Practice Areas']/parent::header/parent::section/ul/li)["+i+"]")).getText();
				if (i<=2) {
//					
					if (i==1) {
...
//		Admissions
		try {
			...
			String main = "";
			int LangSize = driver.findElements(By.xpath("//h1[normalize-space()='Admissions']/parent::header/parent::section/ul/li")).size();
			
			for (int i = 1; i < LangSize+1; i++) {
				String bar = driver.findElement(By.xpath("(//h1[normalize-space()='Admissions']/parent::header/parent::section/ul/li)["+i+"]")).getText();
...
//		Education
		try {
			...
			String main = "";
			int eduSize = driver.findElements(By.xpath("(//h1[normalize-space()='Education']/parent::header/parent::section/ul/li)")).size();
			for (int i = 1; i < eduSize+1; i++) {
				String edu = driver.findElement(By.xpath("(//h1[normalize-space()='Education']/parent::header/parent::section/ul/li)["+i+"]")).getText();
				if (i<=5) {
					if (i==1) {
						sm.setEdu1(edu);
					}else if (i==2) {
...
////		Image 
		try {
			String img = driver.findElement(By.xpath("//div[@class='bio-img']/div/img")).getAttribute("src");
			sm.setImg(img);
		} catch (Exception e) {
			// TODO: handle exception
		}
...
//		Bio
		try {
			StringBuilder bio = new StringBuilder();
				bio.append(driver.findElement(By.xpath("//div[@class='intro']/p")).getText());
				try {

					bio.append(driver.findElement(By.xpath("//div[@class='p blog-article']")).getText());
				} catch (Exception e) {
					// TODO: handle exception
				}
