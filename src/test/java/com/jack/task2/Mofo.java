...
//	String sheetName = "Sheet1";
	String filename = "D:\\Fiverr Work\\Jack Project\\mofo\\input.xlsx";
...
//		driver.manage().deleteAllCookies();
		...
		sm.setLink(link);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h1[@class='profile-hero__headline']")));
	...
	//	Name
		try {
			String name = driver.findElement(By.xpath("//h1[@class='profile-hero__headline']")).getText();
			sm.setName(name);
		} catch (Exception e) {
...
//		practice
		try {
			...
			String main = "";
			int practiceSize = driver.findElements(By.xpath("(//p[normalize-space()='Practices']/following-sibling::ul/li)")).size();
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("(//p[normalize-space()='Practices']/following-sibling::ul/li)["+i+"]")).getText();
				if (i<=2) {
//					
					if (i==1) {
...
//		All
		...
		try {
			StringBuilder bio = new StringBuilder();
				bio.append(driver.findElement(By.xpath("//div[@class='dangerously-set-container default']")).getText());
			sm.setBio(bio);
		} catch (Exception e) {
			// TODO: handle exception
		}
...
//		Admissions
		try {
			String main = "";
			int LangSize = driver.findElements(By.xpath("(//p[normalize-space()='Bar Admissions']/following-sibling::ul/li)")).size();
				for (int i = 1; i < LangSize+1; i++) {
					String bar = driver.findElement(By.xpath("(//p[normalize-space()='Bar Admissions']/following-sibling::ul/li)["+i+"]")).getText();
					if (i<=1) {				
						sm.setBar1(bar);
					}else {
						main+=";"+bar;
...
//		Education
		try {
			...
			String main = "";
			int eduSize = driver.findElements(By.xpath("(//p[normalize-space()='Education']/following-sibling::ul/li)")).size();
			for (int i = 1; i < eduSize+1; i++) {
				String edu = driver.findElement(By.xpath("(//p[normalize-space()='Education']/following-sibling::ul/li)["+i+"]")).getText();
				if (i<=5) {
					if (i==1) {
						sm.setEdu1(edu);
					}else if (i==2) {
...
////		Image 
		try {
			String img = driver.findElement(By.xpath("//div[@class='profile-hero__image-inner']/span/img")).getAttribute("src");
			sm.setImg(img);
		} catch (Exception e) {
			// TODO: handle exception
		}
	...
	public void saveList() throws Exception {
		...
		mapper.writeValue(new File("D:\\Fiverr Work\\Jack Project\\mofo\\row.json"), lists);
	}
