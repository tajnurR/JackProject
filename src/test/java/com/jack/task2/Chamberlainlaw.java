...
//		driver.manage().deleteAllCookies();
		...
		sm.setLink(link);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h1[@id='bioPageTitle']")));
	...
	//	Name
		try {
			String name = driver.findElement(By.xpath("//h1[@id='bioPageTitle']")).getText();
			sm.setName(name);
...
//		//	Title
		try {
			String title = driver.findElement(By.xpath("//div[@id='bioTitle']")).getText();
			sm.setTitle(title);
		} catch (Exception e) {
...
//		//	Email
		try {
			String email = driver.findElement(By.xpath("//li[@id='bioEmail']/a")).getText();
			sm.setEmail(email);
		} catch (Exception e) {
...
//		
		//	Phone
		try {
			String phone = driver.findElement(By.xpath("//span[@itemprop='telephone']")).getText();
			sm.setPhone(phone);
		} catch (Exception e) {
...
//		Location
		try {
			String main = "";
			int locationSize = driver.findElements(By.xpath("(//div[@id='bioOfficeLinks']/a)")).size();
			for (int i = 1; i < locationSize+1; i++) {
				String location = driver.findElement(By.xpath("(//div[@id='bioOfficeLinks']/a)["+i+"]")).getText();
				if (i<=1) {			
...
//		practice
		try {
			...
			String main = "";
			int practiceSize = driver.findElements(By.xpath("(//h3[normalize-space()='Practice Areas']/parent::div/div/ul/li")).size();
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("(//h3[normalize-space()='Practice Areas']/parent::div/div/ul/li)["+i+"]")).getText();
				if (i<=2) {
//					
					if (i==1) {
...
//		try {
//			String main = "";
//			int LangSize = driver.findElements(By.xpath("//td[normalize-space()='Bar Admissions']/following-sibling::td/ul/li")).size();
//			
//			for (int i = 1; i < LangSize+1; i++) {
//				String bar = driver.findElement(By.xpath("(//td[normalize-space()='Bar Admissions']/following-sibling::td/ul/li)["+i+"]")).getText();
//				
//				if (i<=1) {				
//					sm.setBar1(bar);
//				}else {
...
//		
		...
		//	Bar Admissions
		try {
			String bar = driver.findElement(By.xpath("//div[@id='subContent']")).getAttribute("innerHTML");
			sm.setBar1(bar);
		} catch (Exception e) {
...
//				
//			String main = "";
//			int eduSize = driver.findElements(By.xpath("(//td[normalize-space()='Education']/following-sibling::td/ul/li)")).size();
//			for (int i = 1; i < eduSize+1; i++) {
//				String edu = driver.findElement(By.xpath("(//td[normalize-space()='Education']/following-sibling::td/ul/li)["+i+"]")).getText();
//				if (i<=5) {
//					if (i==1) {
//						sm.setEdu1(edu);
//					}else if (i==2) {
...
//		try {
//			String main = "";
//			int eduSize = driver.findElements(By.xpath("//td[normalize-space()='Languages']/following-sibling::td/ul/li")).size();
//			for (int i = 1; i < eduSize+1; i++) {
//				String edu = driver.findElement(By.xpath("//td[normalize-space()='Languages']/following-sibling::td/ul/li["+i+"]")).getAttribute("innerHTML");
//					main+=";"+edu;
//			}
//			sm.setLanguage(main);
//		
...
////		Image 
		try {
			String img = driver.findElement(By.xpath("//img[@id='bioPhoto']")).getAttribute("src");
			sm.setImg(img);
		} catch (Exception e) {
			// TODO: handle exception
		}
...
//		Bio
		try {
			StringBuilder bio = new StringBuilder();
				bio.append(driver.findElement(By.xpath("//section[@id='Profile-tab']")).getText());
//				try {
//
