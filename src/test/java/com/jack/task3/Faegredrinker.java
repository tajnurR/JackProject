...
public class Faegredrinker {
	...
	String filename = "D:\\Fiverr Work\\Jack Project 2\\Faegre Drinker 9\\input.xlsx";
...
//		Bio
		try {
			...
			StringBuilder bio = new StringBuilder();
				bio.append(driver.findElement(By.xpath("(//div[@class='inner-limit tabs-sections']/div/div[@class='section'])[1]")).getAttribute("innerHTML"));
				
				try {
					driver.findElement(By.xpath("//span[@class='tabs-list-title ng-binding'][normalize-space()='Experience']/parent::button")).click();
					Thread.sleep(1000);
...
//		practice
		try {
			...
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("(//h4[normalize-space()='Related Services']/following-sibling::div[@class='tagslist']/a)["+i+"]")).getAttribute("innerHTML");
				if (i<=2) {
					if (i==1) {
...
//		Education
		...
		//Img
		try {
			String img = driver.findElement(By.xpath("//div[contains(@class,'hero-image--professional')]")).getAttribute("style");
			sm.setImg(img);
		} catch (Exception e) {
			// TODO: handle exception
		}
	...
	public void saveList() throws Exception {
		...
		mapper.writeValue(new File("D:\\Fiverr Work\\Jack Project 2\\Faegre Drinker 9\\row.json"), lists);
