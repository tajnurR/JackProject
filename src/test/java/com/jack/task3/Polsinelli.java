...
public class Polsinelli {
	...
	String filename = "D:\\Fiverr Work\\Jack Project 2\\Polsinelli PC 7\\input.xlsx";
...
//		driver.manage().deleteAllCookies();
	...
	//	Name
		try {
			String name = driver.findElement(By.xpath("//h1[@id='page-title']")).getText();
			sm.setName(name);
		} catch (Exception e) {
...
//		Bio
		try {
			...
			StringBuilder bio = new StringBuilder();
				bio.append(driver.findElement(By.xpath("//div[@aria-labelledby='page-title']/div/div/div[1]")).getText());
			sm.setBio(bio);
		} catch (Exception e) {
...
//		practice
		try {
			String main = "";
			int practiceSize = driver.findElements(By.xpath("//h3[normalize-space()='Capabilities']/following-sibling::ul/li")).size();
			for (int i = 1; i < practiceSize+1; i++) {
				String practice = driver.findElement(By.xpath("//h3[normalize-space()='Capabilities']/following-sibling::ul/li["+i+"]")).getText();
				if (i<=2) {
					if (i==1) {
...
//		Admissions
		try {
			String main = "";
			int LangSize = driver.findElements(By.xpath("//h3[normalize-space()='Bar Admission']/following-sibling::ul/li")).size();
			
			for (int i = 1; i < LangSize+1; i++) {
				String bar = driver.findElement(By.xpath("//h3[normalize-space()='Bar Admission']/following-sibling::ul/li["+i+"]")).getText();
...
//		Location
		try {
			String main = "";
			int LangSize = driver.findElements(By.xpath("//div[contains(@class,'type-office')]")).size();
			
			for (int i = 1; i < LangSize+1; i++) {
				String bar = driver.findElement(By.xpath("//div[contains(@class,'type-office')]["+i+"]/a[1]")).getText();
	...
	public void saveList() throws Exception {
		...
		mapper.writeValue(new File("D:\\Fiverr Work\\Jack Project 2\\Polsinelli PC 7\\row.json"), lists);
